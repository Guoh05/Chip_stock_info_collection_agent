...
The `<!-- BEGIN AUTO:status --> … <!-- END AUTO:status -->` block at the top is regenerated by `scraper/scripts/_update_readme_status.py`. It is a **production-facing** snapshot: it renders only from the most recent `production/scraper/BatchTest_*/` folder, never from `test/scraper/`. When no prod batch exists yet it is a **no-op** — the existing block is left untouched rather than blanked. The rest of this file is hand-written. The regenerator runs:

- automatically at the end of every **`--env prod`** `batch_scraper_test.py` invocation (best-effort, never blocks the batch); test/dev runs do **not** refresh it;
- automatically via the Claude Code PostToolUse hook in `.claude/settings.json` whenever files under `scraper/` change in-session (same prod-only scan + no-op-when-empty, so an in-session edit never clobbers the snapshot);
