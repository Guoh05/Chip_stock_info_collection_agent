...
5. **READMEs auto-refresh (`--env prod` only).** The `<!-- BEGIN AUTO:status
   --> ... <!-- END AUTO:status -->` block in each track's README is
   regenerated by the batch driver + a hook **only on `--env prod` runs**;
   `--env test` runs (the default) deliberately leave it untouched so
   smoke/dev batches don't clobber the published status. Do not edit the
   block by hand. Hand-written sections of the README are fair game.
